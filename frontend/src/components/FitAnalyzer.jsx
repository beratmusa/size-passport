...
// ZOOM KOORDİNATLARI (ÜST VE ALT İÇİN AYRI)
const ZOOM_CONFIG = {
  top: {
  ...
  bottom: {
    ...
    hip: "50 200 200 200",
    inseam: "80 250 140 300", // Bacak arasına odaklan
    outseam: "0 200 300 400", // Tüm bacağı göster
    full: "0 150 300 450"     // Belden aşağısını göster
  }
};

const FitAnalyzer = ({ userProfile, onClose, onUpdateProfile, productData }) => {
  if (!productData) return null; // Ürün verisi yoksa hiçbir şey gösterme
  const [activeZone, setActiveZone] = useState(null);
  const svgControls = useAnimation();
  
  // Ürün kategorisini belirle (Varsayılan: top)
  const category = productData.category === 'bottom' ? 'bottom' : 'top';
  const coords = ZOOM_CONFIG[category];

  // GENEL ANALİZ FONKSİYONU
  const analyzeFit = (userVal, garmentVal) => {
    ...
    // Alt giyimde tolerans biraz daha farklı olabilir ama şimdilik aynı mantık
  ...
  // KATEGORİYE GÖRE SONUÇLARI HAZIRLA
  let listItems = [];
  let results = {};
  ...
  if (category === 'top') {
    ...
    results = {
      waist: analyzeFit(userProfile?.measurements?.waist_circumference_cm, productData.measurements.waist),
      hip: analyzeFit(userProfile?.measurements?.hip_circumference_cm, productData.measurements.hip),
      inseam: analyzeFit(userProfile?.measurements?.inseam_cm, productData.measurements.inseam),
      outseam: analyzeFit(userProfile?.measurements?.outseam_cm, productData.measurements.outseam),
    };
    listItems = [
      { id: 'waist', name: 'Bel Çevresi', data: results.waist, user: userProfile?.measurements?.waist_circumference_cm, garment: productData.measurements.waist },
      { id: 'hip', name: 'Basen/Kalça', data: results.hip, user: userProfile?.measurements?.hip_circumference_cm, garment: productData.measurements.hip },
      { id: 'inseam', name: 'İç Bacak', data: results.inseam, user: userProfile?.measurements?.inseam_cm, garment: productData.measurements.inseam },
      { id: 'outseam', name: 'Dış Bacak (Boy)', data: results.outseam, user: userProfile?.measurements?.outseam_cm, garment: productData.measurements.outseam },
    ];
  }
  ...
  useEffect(() => {
    const runAnimationSequence = async () => {
      ...
      // Sadece "Dar" olan bölgelere zoom yap
  ...
  }, [svgControls, category]); // Category değişirse animasyonu resetle
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/60 backdrop-blur-sm p-0 md:p-6 font-sans text-zinc-900">
      <motion.div initial={{ y: 50, opacity: 0, scale: 0.95 }} animate={{ y: 0, opacity: 1, scale: 1 }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="bg-white w-full h-full md:max-w-4xl md:h-[80vh] md:rounded-[2rem] flex flex-col overflow-hidden shadow-2xl">
          ...
          <div className="w-full md:w-1/2 flex items-center justify-center p-4 md:p-8 relative min-h-[400px] md:min-h-0">
            <motion.svg initial={{ viewBox: coords.full }} animate={svgControls} className="h-[90%] w-auto relative z-0 drop-shadow-sm">
              ...
              {category === 'top' ? (
                ...
                // ALT GİYİM (PANTOLON) SİLÜETİ
                <>
                  <g className="text-zinc-300 stroke-current fill-none" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                    {/* Bel Hattı */}
                    <path d="M 110 180 Q 150 190 190 180" />
                    {/* Basen ve Bacaklar */}
                    <path d="M 110 180 Q 90 220 100 350 L 85 550 L 115 560 L 145 280" /> {/* Sol Bacak */}
                    <path d="M 190 180 Q 210 220 200 350 L 215 550 L 185 560 L 155 280" /> {/* Sağ Bacak */}
                    {/* Ağ (Crotch) */}
                  ...
                  <g filter="url(#thermal)" className="mix-blend-multiply opacity-70">
                    ...
                    {/* İÇ BACAK (Inseam) - İki bacak arasına çizgi */}
                    ...
                    {/* DIŞ BACAK (Outseam) - Yanlara çizgi */}
            ...
            <div className="flex flex-col gap-3">
              {listItems.map((item, index) => (
                ...
                >
                  ...
                  {/* SOL KISIM (Nokta + Alt Alta Yazılar) */}
        ...
        <div className="flex-none bg-zinc-900 text-white flex flex-col md:flex-row items-start md:items-center justify-between p-6 md:px-8 md:py-6 relative overflow-hidden">
          ...
          <Button variant="secondary" className="relative z-10 w-full md:w-auto h-10 md:h-12 px-8 rounded-full text-xs md:text-sm font-medium transition-all transform hover:scale-105 active:scale-95 shadow-[0_0_20px_rgba(255,255,255,0.1)]">
            Sepete Ekle
          </Button>
        </div>
      </motion.div>
