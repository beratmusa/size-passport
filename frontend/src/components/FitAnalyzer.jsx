      ...
      <motion.div initial={{ y: 50, opacity: 0, scale: 0.95 }} animate={{ y: 0, opacity: 1, scale: 1 }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="bg-white w-full h-full md:max-w-4xl md:h-[80vh] md:rounded-[2rem] flex flex-col overflow-hidden shadow-2xl">
        ...
        <div className="flex-none flex items-center justify-between px-6 py-4 md:px-8 md:py-5 border-b border-zinc-100 bg-white z-10">
          ...
          <div className="flex items-center gap-3 md:gap-4">
            <button onClick={onUpdateProfile} className="text-[10px] md:text-xs font-medium text-zinc-500 hover:text-zinc-900 transition-colors uppercase tracking-wider border border-zinc-200 px-3 py-1.5 md:px-4 md:py-2 rounded-full hover:bg-zinc-50 hidden sm:block">
              Pasaportu Güncelle
            </button>
            <button onClick={onClose} className="p-2 rounded-full hover:bg-zinc-100 transition-colors text-zinc-400 hover:text-zinc-800">
              ...
              {[
                ...
                >
                  <div className="flex items-center gap-3 w-1/3">
                    <div className={`w-3 h-3 rounded-full ${item.data.bg} shadow-inner`}></div>
                    <span className="text-xs md:text-sm font-medium text-zinc-700">{item.name}</span>
                  </div>
                  <div className="flex items-center gap-3 text-[10px] md:text-xs text-zinc-400 font-mono w-1/3 justify-center">
                    <span title="Senin Ölçün">{item.user || '?'}</span>
                    ...
                    <span title="Ürün Ölçüsü" className="text-zinc-600 font-semibold">{item.garment}</span>
                  </div>
                  <div className="w-1/3 flex justify-end">
                    <span className="text-[10px] uppercase tracking-widest px-2.5 py-1 rounded-md font-semibold bg-zinc-50 border border-zinc-100" style={{ color: item.data.color }}>
        ...
        <div className="flex-none bg-zinc-900 text-white flex flex-col md:flex-row items-start md:items-center justify-between p-6 md:px-8 md:py-6 relative overflow-hidden">
          ...
          <button className="relative z-10 w-full md:w-auto bg-white hover:bg-zinc-200 text-zinc-900 text-xs md:text-sm font-medium py-3 px-6 rounded-full transition-all transform hover:scale-105 active:scale-95 whitespace-nowrap shadow-[0_0_20px_rgba(255,255,255,0.1)]">
            Sepete Ekle
          </button>
        </div>
      </motion.div>
