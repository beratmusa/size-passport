...
const GuideCard = ({ title, desc }) => (
  <div className="bg-white border border-zinc-200 rounded-xl md:rounded-2xl p-4 md:p-6 shadow-sm flex flex-col">
    ...
    <div className="w-full h-20 md:h-24 bg-[#F9F9F9] border border-dashed border-zinc-300 rounded-lg md:rounded-xl flex items-center justify-center">
      <span className="text-[8px] md:text-[10px] text-zinc-400 uppercase tracking-widest text-center px-2">Image Area</span>
    </div>
  </div>
);

const MeasurementWizard = ({ session }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    measurements: {
      ...
      hip_circumference_cm: '', arm_length_cm: '', inseam_cm: '', outseam_cm: '',
    },
  ...
  const handleBasicInfoChange = (e) => {
    ...
    setFormData((prev) => ({ ...prev, basic_info: { ...prev.basic_info, [name]: value === '' ? '' : Number(value) } }));
  };

  const handleMeasurementChange = (e) => {
    const { name, value } = e.target;
    setFormData((prev) => ({ ...prev, measurements: { ...prev.measurements, [name]: value === '' ? '' : Number(value) } }));
  };
  ...
  return (
    <div className="min-h-screen lg:h-screen w-full bg-[#F6F6F6] flex flex-col font-sans text-zinc-900 lg:overflow-hidden selection:bg-zinc-200">
      
      <header className="flex-none w-full py-4 px-4 md:px-8 bg-white border-b border-zinc-200 z-10 flex flex-col lg:flex-row items-center justify-between gap-4">
        
        <div className="text-center lg:text-left w-full lg:w-auto flex items-center justify-between lg:justify-start gap-4">
          <div>
            <h2 className="text-lg md:text-xl font-medium text-zinc-800 tracking-tight">
              {t('app_title')}
            </h2>
            <p className="text-zinc-500 text-[10px] md:text-xs font-light mt-1 hidden sm:block">
              {t('app_subtitle')}
            </p>
          </div>
          ...
          <button 
            ...
            className="lg:ml-4 px-3 py-1 text-[10px] font-bold tracking-widest bg-zinc-100 hover:bg-zinc-200 text-zinc-700 rounded-md transition-colors uppercase border border-zinc-300"
          >
            {i18n.language === 'tr' ? 'EN' : 'TR'}
          </button>
          <button onClick={handleLogout} className="px-3 py-1 text-[10px] font-bold tracking-widest bg-red-50 hover:bg-red-100 text-red-600 rounded-md transition-colors uppercase border border-red-200">
              Çıkış
            </button>
        </div>

        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 w-full lg:w-auto">
          <div className="flex items-center gap-2 md:gap-4 bg-[#F9F9F9] px-3 md:px-5 py-1.5 md:py-2 rounded-full border border-zinc-200 shadow-sm w-full sm:w-auto justify-center">
          ...
          <div className="flex gap-2 w-full sm:w-auto justify-center">
            <button onClick={() => handleGenderChange('kadin')} className={`flex-1 sm:flex-none py-1.5 md:py-2 px-4 md:px-6 rounded-full text-[8px] md:text-[10px] font-medium tracking-widest uppercase transition-all duration-300 border ${formData.gender === 'kadin' ? 'bg-zinc-900 text-white border-zinc-900 shadow-sm' : 'bg-transparent text-zinc-500 border-zinc-300 hover:border-zinc-400'}`}>{t('gender_female')}</button>
            <button onClick={() => handleGenderChange('erkek')} className={`flex-1 sm:flex-none py-1.5 md:py-2 px-4 md:px-6 rounded-full text-[8px] md:text-[10px] font-medium tracking-widest uppercase transition-all duration-300 border ${formData.gender === 'erkek' ? 'bg-zinc-900 text-white border-zinc-900 shadow-sm' : 'bg-transparent text-zinc-500 border-zinc-300 hover:border-zinc-400'}`}>{t('gender_male')}</button>
          </div>
        </div>
      </header>

      <div className="flex-1 flex flex-col lg:flex-row w-full max-w-[1600px] mx-auto overflow-y-auto lg:overflow-hidden">
        ...
        <div className="w-full lg:w-5/12 xl:w-1/3 p-4 sm:p-6 lg:p-10 order-2 lg:order-1 lg:overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          ...
          <div className="space-y-4 md:space-y-6 pb-10 lg:pb-20">
            {/* Şimdilik Guide kartlarını statik bıraktık, ileride i18n dosyasına alınabilir */}
        ...
        <div className="w-full lg:w-7/12 xl:w-2/3 bg-white lg:border-l border-zinc-200 p-2 sm:p-4 lg:p-8 flex flex-col items-center justify-start lg:justify-center relative order-1 lg:order-2 shadow-sm lg:shadow-[-10px_0_30px_rgba(0,0,0,0.02)] border-b lg:border-b-0 pb-8 lg:pb-0">
          ...
          <div className="relative w-full max-w-xl h-[380px] sm:h-[460px] lg:h-[500px] flex items-center justify-center mt-4 lg:mt-0">
            ...
            <div className="absolute top-[18%] left-[0%] md:left-[8%] flex items-center group">
              <div className="mr-1 md:mr-2 flex flex-col items-end">
                ...
                <div className="flex items-baseline border-b border-zinc-300 pb-0.5 md:pb-1">
              ...
              <div className="w-6 md:w-20 border-b border-zinc-300"></div>
            </div>
            ...
            <div className="absolute top-[38%] left-[0%] md:left-[8%] flex items-center group">
              ...
              <div className="w-8 md:w-24 border-b border-zinc-300"></div>
            </div>
            ...
            <div className="absolute top-[55%] left-[0%] md:left-[8%] flex items-center group">
              ...
              <div className="w-8 md:w-20 border-b border-zinc-300"></div>
            </div>
            ...
            <div className="absolute top-[72%] left-[0%] md:left-[8%] flex items-center group">
              ...
              <div className="w-10 md:w-24 border-b border-zinc-300"></div>
            </div>
            ...
            <div className="absolute top-[28%] right-[0%] md:right-[8%] flex items-center flex-row-reverse group">
              ...
              <div className="w-6 md:w-16 border-b border-zinc-300"></div>
            </div>
            ...
            <div className="absolute top-[48%] right-[0%] md:right-[8%] flex items-center flex-row-reverse group">
              ...
              <div className="w-3 md:w-10 border-b border-zinc-300"></div>
            </div>
            ...
            <div className="absolute top-[65%] right-[0%] md:right-[8%] flex items-center flex-row-reverse group">
              ...
              <div className="w-12 md:w-24 border-b border-zinc-300"></div>
            </div>
          ...
          <div className="w-full max-w-lg mt-4 md:mt-6 flex flex-col items-center gap-3 md:gap-4 px-2">
            ...
            <button onClick={handleSave} disabled={isSubmitting} className="w-full bg-zinc-900 hover:bg-zinc-800 text-white font-medium py-3.5 md:py-4 px-8 rounded-full transition-all duration-300 transform hover:scale-105 active:scale-95 flex items-center justify-center gap-2 uppercase tracking-widest text-[10px] md:text-[11px] shadow-lg mt-1 md:mt-2">
              {isSubmitting ? <span className="animate-pulse">{t('saving')}</span> : <>{t('save_button')} <svg className="w-3 h-3 md:w-4 md:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg></>}
            </button>
          </div>
